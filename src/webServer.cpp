...
void WebServer::otherSettingsUpdate(AsyncWebServerRequest *request)
{
	...
	if (request->hasArg(ntpServer1))
	{
		config::instance.data.ntpServer1 = request->arg(ntpServer1);
	}

	if (request->hasArg(ntpServer2))
	{
		config::instance.data.ntpServer2 = request->arg(ntpServer2);
	}

	if (request->hasArg(ntpServerRefreshInterval))
	{
		config::instance.data.ntpServerRefreshInterval = request->arg(ntpServerRefreshInterval).toInt() * 1000;
	}

	if (request->hasArg(timezone))
	{
		config::instance.data.timezone = static_cast<TimeZoneSupported>(request->arg(timezone).toInt());
	}
...
void WebServer::notifySensorChange()
{
	if (events.count())
	{
